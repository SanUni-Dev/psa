  ...
  program_enrollment(frm) {
    ...
    if (frm.doc.program_enrollment) {
      psa_utils.get_program_enrollment(frm.doc.program_enrollment, function (status, enrollment_date, student, academic_program) {
        ...
        if (status == "Continued") {
          psa_utils.get_url_to_new_form("Suspend Enrollment Request", function (url) {
            frm.set_intro((
              `<div class="container">
                                  <div class="row">
                                      <div class="col-auto me-auto">` +
              __(`Can't add a continue enrollment request, because current status is ${status}!` +
                `</div>
                                      <div class="col-auto me-auto">
                                          <a href="${url}">` +
          ...
          psa_utils.get_active_request("Continue Enrollment Request", frm.doc.program_enrollment, function (doc) {
            if (doc) {
              ...
              frm.set_intro((__(`Can't add a continue enrollment request, because you have an active continue enrollment request (`) + url_of_active_request + __(`) that is ${doc.status}!`)), 'red');
            }
            else {
