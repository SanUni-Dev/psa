...
class ExtensionRequest(Document):
	...
	def before_insert(self):
		...
		if not program_enrollment_status[0]:
			frappe.throw(_("Can't add an extension request, because current status is {0}!").format(program_enrollment_status[1]))
			...
			if check_active_requests_before_insert:
				active_request = check_active_request(self.student, self.program_enrollment, ["Extension Request", "Suspend Enrollment Request", "Continue Enrollment Request", "Withdrawal Request"])
				if active_request:
					...
					frappe.throw(
						_("Can't add an extension request, because you have an active {0} ({1}) that is {2}!").format(active_request[0], url_of_active_request, active_request[1]['status'])
					)
