...
function checkFacultyMemberCoLimit(frm, cdt, cdn, selected_faculty_member_id) {
    frappe.call({
        method: "frappe.client.get",
        args: {
            doctype: "PSA Settings",
        ...
        callback: function(response) {
            if (response.message) {
                ...
                let number_of_researches_co = psa_settings.number_of_researches_co;
                let set_limit_on_number_researches = psa_settings.set_limit_on_number_researches_faculty_member_co_supervisor;

                if (set_limit_on_number_researches) {
                    // نستعلم من الدالة للحصول على بيانات النصاب للمشرفين المساعدين
                    frappe.call({
                        method: "psa.tasks.cron.get_supervisor_co_workload",
                                ...
                                frappe.call({
                                    ...
                                    callback: function(member_response) {
                                        ...
                                        var message = frappe._(
                                            ...
                                            [member_name, number_of_researches_co]
                                        );
                                        frappe.msgprint(`
                                            <div style="font-family: Arial, sans-serif; color: #333; background-color: #f8d7da; border: 1px solid #f5c6cb; padding: 15px; border-radius: 5px;">
                                                <p style="font-size: 16px; color: #721c24;">
