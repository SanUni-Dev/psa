                ...
                if (status == "Suspended") {
                    psa_utils.get_url_to_new_form("Continue Enrollment Request", function (url) {
                        frm.set_intro((
                            ...
                            __(`Can't add a change research title request, because current status is ${status}!` +
                                `</div>
                                        <div class="col-auto me-auto">
                                            <a href="${url}">` +
                    ...
                    psa_utils.get_active_request("Suspend Enrollment Request", frm.doc.program_enrollment, function (doc) {
                        if (doc) {
                            ...
                            frm.set_intro((__(`Can't add a change research title request, because you have an active suspend enrollment request (`) + url_of_active_request + __(`) that is ${doc.status}!`)), 'red');
                        }
                        else {
                            psa_utils.get_active_request("Continue Enrollment Request", frm.doc.program_enrollment, function (doc) {
                                if (doc) {
                                    ...
                                    frm.set_intro((__(`Can't add a change research title request, because you have an active continue enrollment request (`) + url_of_active_request + __(`) that is ${doc.status}!`)), 'red');
                                }
                                else {
